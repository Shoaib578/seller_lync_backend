...
def GetAllCategoriesForNormalUsers():
    ...
    get_categories_sql = text("SELECT *,(SELECT count(*) FROM vendors WHERE  vendor_zip_code='"+str(user_default_location)+"' OR city_name LIKE '%"+str(user_default_location)+"%' AND vendor_category=vendor_category_id ) as home_page_vendors_count  from vendor__categories")
...
def view_category_by_user_default_location():
    ...
    vendors_sql = text("Select * from vendors WHERE  vendor_zip_code='"+str(user_default_location)+"' OR city_name LIKE '%"+str(user_default_location)+"%' AND vendor_category="+str(category_id)+"")
    ...
    category_sql = text("Select *,(SELECT count(*) FROM vendors WHERE  vendor_zip_code='"+str(user_default_location)+"' OR city_name LIKE '%"+str(user_default_location)+"%' AND vendor_category=vendor_category_id ) as vendors_count from vendor__categories where vendor_category_id="+str(category_id)+"")
...
def ViewSearchedCategory():
    ...
    category_sql = text("Select *,(SELECT count(*) FROM vendors WHERE  vendor_zip_code='"+str(location)+"' OR city_name LIKE '%"+str(location)+"%' AND vendor_category=vendor_category_id ) as vendors_count from vendor__categories where vendor_category_id="+str(category_id)+"")
    category_query = db.engine.execute(category_sql)
    category_schema = Vendor_CategoriesSchema(many=True)
    category_info = category_schema.dump(category_query)
...
def dropdown_vendors():
    ...
    vendors_sql = text("Select * from vendors WHERE  vendor_zip_code='"+str(location)+"' OR city_name LIKE '%"+str(location)+"%' AND vendor_category="+str(category_id)+" LIMIT 0, 5")
