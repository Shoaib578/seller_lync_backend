...
def GetAllCategoriesForNormalUsers():
    ...
    get_categories_sql = text("SELECT *,(SELECT count(*) FROM vendors WHERE  vendor_category=vendor_category_id AND  vendor_city_name ='"+str(user_default_location)+"') as home_page_vendors_count  from vendor__categories")
...
def view_category_by_user_default_location():
    ...
    vendors_sql = text("Select * from vendors WHERE vendor_category="+str(category_id)+" AND  vendor_city_name ='"+str(user_default_location)+"'")
    ...
    category_sql = text("Select *,(SELECT count(*) FROM vendors WHERE vendor_category=vendor_category_id  OR vendor_city_name ='"+str(user_default_location)+"') as vendors_count from vendor__categories where vendor_category_id="+str(category_id)+"")
...
def GetAllCategoriesBySearch():
    ...
    get_categories_sql = text("SELECT *,(SELECT count(*) FROM vendors WHERE vendor_category=vendor_category_id AND  vendor_city_name='"+str(location)+"') as search_page_vendors_count  from vendor__categories WHERE category LIKE'%"+str(item)+"%'")
    get_categories_query = db.engine.execute(get_categories_sql)
    vendor_categories_schema = Vendor_CategoriesSchema(many=True)
    all_categories =vendor_categories_schema.dump(get_categories_query)
...
def ViewSearchedCategory():
    ...
    category_sql = text("Select *,(SELECT count(*) FROM vendors WHERE  vendor_category=vendor_category_id AND   vendor_city_name ='"+str(location)+"') as vendors_count from vendor__categories where vendor_category_id="+str(category_id)+"")
    category_query = db.engine.execute(category_sql)
    category_schema = Vendor_CategoriesSchema(many=True)
    category_info = category_schema.dump(category_query)
